            ...
            "24h" => {
                ...
                let rows = stmt.query_map(rusqlite::params![threshold_ms], |row| {
                    Ok(ProviderStat {
                        ...
                        cache_triggered_count: row.get(6)?,
                        cache_hit_count: row.get(7)?,
                        sum_ttfb_ms: row.get(8)?,
                        sum_duration_ms: row.get(9)?,
                    })
                })?;
                rows.collect()
            }
...
mod tests {
    ...
    fn test_query_provider_stats_24h() {
        ...
        // provider-a: 2 条成功，provider-b: 1 条失败
...

        {
            ...
            conn.execute(
                "INSERT INTO daily_rollups (provider_name, rollup_date, request_count, success_count,
                    ...
                    total_cache_read_tokens, cache_triggered_count, cache_hit_count,
                    sum_ttfb_ms, sum_duration_ms)
                 VALUES ('provider-a', ?1, 10, 9, 1000, 500, 0, 0, 0, 0, 1000, 5000)",
                rusqlite::params![d1],
            ).unwrap();
            conn.execute(
                "INSERT INTO daily_rollups (provider_name, rollup_date, request_count, success_count,
                    ...
                    total_cache_read_tokens, cache_triggered_count, cache_hit_count,
                    sum_ttfb_ms, sum_duration_ms)
                 VALUES ('provider-a', ?1, 5, 5, 500, 200, 0, 0, 0, 0, 500, 2000)",
                rusqlite::params![d2],
            ).unwrap();
            // provider-b 一天数据
            conn.execute(
