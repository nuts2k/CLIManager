...
mod tests {
    ...
    async fn test_text_delta_full_sequence() {
        let chunks = vec![
            make_chunk(
                r#"{"id":"chatcmpl-1","model":"gpt-4o","choices":[{"delta":{"content":"Hello"}}]}"#,
            ),
            make_chunk(
                r#"{"id":"chatcmpl-1","model":"gpt-4o","choices":[{"delta":{"content":" world"}}]}"#,
            ),
            make_chunk(
        ...
        assert!(
            events.iter().any(|e| e.get("type").and_then(|v| v.as_str())
    ...
    async fn test_tool_deferred_start() {
        let chunks = vec![
            ...
            make_chunk(
                r#"{"id":"chatcmpl-2","model":"gpt-4o","choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"{\"a\":"}}]}}]}"#,
            ),
            // 第二个 chunk 带来 id 和 name
            make_chunk(
                r#"{"id":"chatcmpl-2","model":"gpt-4o","choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_0","type":"function","function":{"name":"my_tool"}}]}}]}"#,
            ),
            // 第三个 chunk 继续 arguments
            make_chunk(
                r#"{"id":"chatcmpl-2","model":"gpt-4o","choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"1}"}}]}}]}"#,
            ),
            make_chunk(
                r#"{"id":"chatcmpl-2","model":"gpt-4o","choices":[{"delta":{},"finish_reason":"tool_calls"}]}"#,
            ),
            done_chunk(),
        ];
    ...
    async fn test_multi_tool_concurrent() {
        let chunks = vec![
            make_chunk(
                r#"{"id":"chatcmpl-3","model":"gpt-4o","choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_0","type":"function","function":{"name":"first_tool"}}]}}]}"#,
            ),
            make_chunk(
                r#"{"id":"chatcmpl-3","model":"gpt-4o","choices":[{"delta":{"tool_calls":[{"index":1,"id":"call_1","type":"function","function":{"name":"second_tool"}}]}}]}"#,
            ),
            make_chunk(
                r#"{"id":"chatcmpl-3","model":"gpt-4o","choices":[{"delta":{"tool_calls":[{"index":1,"function":{"arguments":"{\"b\":2}"}}]}}]}"#,
            ),
            make_chunk(
                r#"{"id":"chatcmpl-3","model":"gpt-4o","choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"{\"a\":1}"}}]}}]}"#,
            ),
            make_chunk(
                r#"{"id":"chatcmpl-3","model":"gpt-4o","choices":[{"delta":{},"finish_reason":"tool_calls"}],"usage":{"prompt_tokens":8,"completion_tokens":4}}"#,
            ),
            done_chunk(),
        ];
        ...
        assert!(
            deltas
                .iter()
                .any(|(idx, json)| *idx == call0_idx && json == "{\"a\":1}"),
            "call_0 的参数应路由到其 index"
        );
        assert!(
            deltas
                .iter()
                .any(|(idx, json)| *idx == call1_idx && json == "{\"b\":2}"),
    ...
    async fn test_stream_end_closes_all_blocks() {
        let chunks = vec![
            make_chunk(
                r#"{"id":"chatcmpl-4","model":"gpt-4o","choices":[{"delta":{"content":"Hi"}}]}"#,
            ),
            make_chunk(
                r#"{"id":"chatcmpl-4","model":"gpt-4o","choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_x","type":"function","function":{"name":"tool_x"}}]}}]}"#,
            ),
            make_chunk(
                r#"{"id":"chatcmpl-4","model":"gpt-4o","choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"{}"}}]}}]}"#,
            ),
            make_chunk(
                r#"{"id":"chatcmpl-4","model":"gpt-4o","choices":[{"delta":{},"finish_reason":"tool_calls"}],"usage":{"prompt_tokens":10,"completion_tokens":5}}"#,
            ),
            done_chunk(),
        ];
        ...
        assert!(
            ...
            "stop({stop_count}) 应 >= start({start_count})"
        );
    ...
    async fn test_finish_reason_length_maps_to_max_tokens() {
        let chunks = vec![
            make_chunk(
                r#"{"id":"chatcmpl-6","model":"gpt-4o","choices":[{"delta":{"content":"..."}}]}"#,
            ),
            make_chunk(
                r#"{"id":"chatcmpl-6","model":"gpt-4o","choices":[{"delta":{},"finish_reason":"length"}]}"#,
            ),
            done_chunk(),
        ];
