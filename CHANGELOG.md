...
### 修复
- [traffic] 请求总数从 DB 精确查询，不再受 500 条内存上限限制

## v0.2.14 (2026-03-20)

### 修复
- [ship] 修正环境变量传递与变更日志校验

### 重构
- [ui] 移除监控与设置页冗余标题

### 文档
- [plans] 补充页面标题精简方案与实施计划
- [changelog] 补充 v0.2.13 发布说明

## v0.2.13 (2026-03-19)

### 修复
- 修正统计页错误态并在刷新失败时保留上一次成功加载的数据
- 结束异常流式请求的 Pending 状态，未收到最终 usage 时补写失败终态
- 趋势图 Token 总量计入缓存 token 并统一 K/M 显示
- 流量趋势图横坐标改用本地时间替代 UTC
- 缓存命中率改为 token 级别计算
- Token 列计入缓存 token 以反映完整计费量

### 其他
- 同步本地版本元数据与 worktree 忽略规则

## v0.2.12 (2026-03-19)

### 新功能
- 将页面切换改为 Header 内导航标签（主页/监控/设置），带图标+文字+选中指示线

### 修复
- 优化统计分析页布局：排行榜从双列并排改为各占一行，解决数据显示不全
- 统一统计分析页滚动条样式（scrollbar-thin），与实时日志表格保持一致
- 时间范围切换时保留旧数据展示，消除图表闪烁

### 其他
- 同步 Cargo.lock 版本号，修正发版流程中的版本元数据
## v0.2.11 (2026-03-19)
