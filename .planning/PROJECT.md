...
- [ ] Claude settings.json overlay：编辑 / 校验 / 保存（iCloud 同步优先，本地降级）
- [ ] Overlay 深度合并（含 null 删除）+ 保护字段（Provider/Proxy 永远优先）
- [ ] 自动应用：保存即 apply + 启动对齐 + iCloud watcher 变更自动 apply

## Shipped: v2.4 Anthropic 模型映射 (2026-03-15)

**Delivered:** Anthropic 协议透传路径完整支持模型映射，代理层自动替换请求 model 字段并反向映射响应/流式 SSE，Provider 编辑 UI 统一显示映射配置

## Current Milestone: v2.5 Claude 全局配置 Overlay

**Goal:** 在 CLIManager 内维护 Claude Code 的全局 settings.json overlay（深度合并 + null 删除），并在不破坏 surgical patch 的前提下自动对齐到 `~/.claude/settings.json`（含 iCloud 同步 watcher）。

**Target features:**
- Settings → Advanced 增加 Claude 小节：JSON overlay 编辑 / 校验 / 保存 / 立即应用
- 深度合并规则：object 递归合并、array 整体替换、scalar 覆盖、null 删除 key
- 保护字段永远优先：overlay 不得覆盖 `env.ANTHROPIC_AUTH_TOKEN` / `env.ANTHROPIC_BASE_URL`
- 存储：iCloud 可用写入 iCloud config 目录，不可用降级本地目录；并提示是否同步
- 触发：保存设置立即 apply、应用启动时 best-effort apply、iCloud 文件变更 watcher 自动 apply

## Next Milestone

候选方向：
- **v2.6 流量监控**：代理请求日志、流量统计可视化
...
**Current State (v2.4 shipped 2026-03-15):**
- ~19,600 LOC (Rust + TypeScript/React)
...
- 23 phases, 48 plans across 7 milestones
...
- 367 Rust lib tests, all passing
...
---
*Last updated: 2026-03-16 after starting v2.5 Claude 全局配置 Overlay milestone*
