...
- [ ] 本地代理服务随应用启动运行
- [ ] 按 CLI 固定端口监听（Claude Code、Codex 各自端口）
- [ ] 双模式切换：直连模式 vs 代理模式（全局总开关 + 每 CLI 独立开关）
- [ ] 代理模式下 CLI 配置自动 patch 指向 localhost:port
- [ ] 代理模式下切换 Provider 实时生效，无需重启 CLI
- [ ] 直连模式下保持现有 surgical patch 行为
- [ ] 总开关放设置页，CLI 独立开关在主窗口对应 Tab 内显示
- [ ] 代理设置存本地（不同步），首次运行默认关闭
...
**Current State (v1.1 shipped 2026-03-13):**
- 8,441 LOC (5,411 Rust + 3,030 TypeScript/React)
- Tech stack: Tauri 2.10, React 19, Vite 7, shadcn/ui, Tailwind CSS v4, i18next
- Rust crates: serde, serde_json (preserve_order), toml_edit, notify (FSEvents), uuid, chrono, reqwest
- 7 phases, 15 plans across 2 milestones
- System tray with dynamic provider menu, close-to-tray lifecycle, auto-refresh from all state change sources

**Known Issues:**
- Claude Code and Codex config schemas need verification against current versions (flagged by research)
- iCloud file eviction detection from Rust may need objc2 bindings or Swift bridge (flagged by research)
- Release build tray behavior may differ from dev build (needs verification)

## Current Milestone: v2.0 Local Proxy

**Goal:** 在应用内运行本地代理服务，CLI 指向 localhost 实现实时 Provider 切换，无需修改配置文件或重启 CLI。

**Target features:**
- 本地 HTTP 代理服务（随应用运行，按 CLI 固定端口）
- 双模式切换（直连 vs 代理），全局总开关 + 每 CLI 独立开关
- 代理模式下 Provider 切换实时生效
- UI 集成（设置页总开关 + Tab 内独立开关）

**远景（2.x+）：** 协议转换（Anthropic↔OpenAI）、OAuth 桥接、流量监控与可视化 → 全功能 API 网关
