...
#### Current Milestone: v2.2 协议转换

**Goal:** 让 Claude Code 通过代理模式使用 OpenAI 兼容的 Provider，代理层自动完成 Anthropic Messages API ↔ OpenAI Chat Completions API 的协议转换。

**Target features:**
- Anthropic → OpenAI 请求转换（Messages API → Chat Completions）
- OpenAI → Anthropic 响应转换（Chat Completions → Messages API）
- 流式 SSE 双向转换
- 工具调用（tool_use）格式转换
- 图片/多模态内容转换
- 代理模式下按 Provider 协议类型自动路由（直传 vs 转换）
...
**Current State (v2.1 shipped 2026-03-14):**
- ~12,000 LOC (Rust + TypeScript/React)
- Tech stack: Tauri 2.10, React 19, Vite 7, shadcn/ui, Tailwind CSS v4, i18next, axum 0.8
- Rust crates: serde, serde_json (preserve_order), toml_edit, notify (FSEvents), uuid, chrono, reqwest (+stream), axum, tower-http, tokio (net,sync,time), tauri-plugin-updater, tauri-plugin-process
- 13 phases, 27 plans across 4 milestones
...
候选方向：
- **v2.3 流量监控**：代理请求日志、流量统计可视化
- **v3.0 全功能网关**：OAuth 桥接、自动 Failover、熔断器、反向协议转换
